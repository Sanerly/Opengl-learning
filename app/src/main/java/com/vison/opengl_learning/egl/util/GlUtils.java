...
public class GlUtils {
     ...
     * @return
     */
    public static synchronized int createProgram(Resources res,String vertexSource, String fragmentSource) {
        int vertexShader = loadShader(GLES20.GL_VERTEX_SHADER, loadFromAssetsFile(res,vertexSource));
        if (vertexShader == 0) {
            return 0;
        }
        int fragmentShader = loadShader(GLES20.GL_FRAGMENT_SHADER, loadFromAssetsFile(res,fragmentSource));
        if (fragmentShader == 0) {
            return 0;
        }
...
//            throw new RuntimeException(msg);
    ...
    public static int createTexture(byte[] bytes, int width, int height) {
        return createTexture(bytes, width, height, GlUtils.GL_NOT_TEXTURE);
    }
